...
## 創建自定義最佳PSM模型生成函數
def create_custom_psm_results(
    # 模型配置參數
    binary_cutoff_method='median',      # 'median', 'mean', 'percentile_75', 'percentile_33'
    matching_method='nearest',          # 'nearest', 'caliper'
    matching_ratio='1:2',              # '1:1', '1:2', '1:3', '1:5'
    caliper_width=0.1,                 # Caliper匹配的容忍度
    with_replacement=False,            # 是否允許重複匹配
    trimming_alpha=0.0,               # 修剪極值的比例
    # 輸出配置
    treatment_focus='both',            # 'family', 'gov', 'both'
    output_filename_suffix='enhanced'  # 輸出檔案後綴
):
    """
    創建自定義配置的PSM分析結果
    
    參數：
    - binary_cutoff_method: 二元化方法
    - matching_method: 匹配方法
    - matching_ratio: 匹配比例
    - caliper_width: Caliper寬度（僅當matching_method='caliper'時使用）
    - with_replacement: 是否允許重複匹配
    - trimming_alpha: 修剪比例
    - treatment_focus: 關注的處理變數 ('family', 'gov', 'both')
    - output_filename_suffix: 輸出檔案後綴
    """
    
    print(f"\n{'='*80}")
    print("創建自定義PSM最佳模型結果")
    print(f"{'='*80}")
    print(f"配置參數：")
    print(f"- 二元化方法：{binary_cutoff_method}")
    print(f"- 匹配方法：{matching_method}")
    ...
    if matching_method == 'caliper':
        print(f"- Caliper寬度：{caliper_width}")
    print(f"- 重複匹配：{with_replacement}")
    print(f"- 修剪比例：{trimming_alpha*100:.1f}%")
    print(f"- 關注變數：{treatment_focus}")
    print(f"{'='*80}")
    
    # 設定模型參數
    custom_params = {
        'binary_cutoff_method': binary_cutoff_method,
        'matching_method': matching_method,
        'matching_ratio': matching_ratio,
        'caliper_width': caliper_width,
        'with_replacement': with_replacement,
        'trimming_alpha': trimming_alpha,
        'robust_se': True,
        'balance_check': True
    }
    ...
    try:
        ...
        # Family分析
        if treatment_focus in ['family', 'both']:
            print(f"\n{'-'*60}")
            print("執行Family變數自定義PSM分析")
            print(f"{'-'*60}")
            
            # 使用前面定義的變數
            control_vars_custom = ['g', 'size', 'lev', 'roa', 'mtb', 'kz', 'boardSize', 'CEOdual', 'CSRcmte']
            outcome_vars_custom = ['gap', 'gap_e', 'gap_s']
            
            family_results_custom, family_matched_custom = perform_enhanced_psm_analysis(
                treatment_var='family',
                data=df_psm.copy(),
                control_vars=control_vars_custom,
                outcome_vars=outcome_vars_custom,
                **custom_params
            )
            
            results_dict['family'] = family_results_custom
        
        # Gov分析
        if treatment_focus in ['gov', 'both']:
            print(f"\n{'-'*60}")
            print("執行Gov變數自定義PSM分析")
            print(f"{'-'*60}")
            
            # 使用前面定義的變數
            control_vars_custom_gov = ['g', 'size', 'lev', 'roa', 'mtb', 'kz', 'boardSize', 'CEOdual', 'CSRcmte']
            outcome_vars_custom_gov = ['gap', 'gap_e', 'gap_s']
            
            gov_results_custom, gov_matched_custom = perform_enhanced_psm_analysis(
                treatment_var='gov',
                data=df_psm_gov.copy(),
                control_vars=control_vars_custom_gov,
                outcome_vars=outcome_vars_custom_gov,
                **custom_params
            )
            
            results_dict['gov'] = gov_results_custom
        
        # 創建結果表格
        if treatment_focus == 'family' or treatment_focus == 'both':
            family_table = create_baseline_table(results_dict['family'], outcome_vars_custom)
            family_filename = f"PSM results/psm_results_{output_filename_suffix}_family.csv"
            family_table.to_csv(family_filename, index=False, encoding='utf-8-sig')
            print(f"\n已輸出Family自定義結果：{family_filename}")
        
        if treatment_focus == 'gov' or treatment_focus == 'both':
            gov_table = create_baseline_table_gov(results_dict['gov'], outcome_vars_custom_gov)
            gov_filename = f"PSM results/psm_results_{output_filename_suffix}_gov.csv"
            gov_table.to_csv(gov_filename, index=False, encoding='utf-8-sig')
            print(f"\n已輸出Gov自定義結果：{gov_filename}")
        
        print(f"\n{'='*80}")
        print("自定義PSM模型分析完成！")
        print(f"配置摘要：{binary_cutoff_method} + {matching_method} + {matching_ratio}")
        if trimming_alpha > 0:
            print(f"修剪比例：{trimming_alpha*100:.1f}%")
        print(f"{'='*80}")
        ...
        print(f"\n自定義PSM分析過程中發生錯誤：{str(e)}")
        import traceback
        traceback.print_exc()
        return None

print(f"\n{'='*100}")
print("自定義PSM最佳模型功能已就緒！")
print(f"{'='*100}")
print("使用方式範例：")
print()
print("# 範例1：基於比較結果，使用表現最佳的1:2匹配配置")
print("custom_results_best = create_custom_psm_results(")
print("    binary_cutoff_method='median',")
print("    matching_method='nearest',")
print("    matching_ratio='1:2',")
print("    with_replacement=False,")
print("    trimming_alpha=0.05,")
print("    treatment_focus='both',")
print("    output_filename_suffix='best_performance'")
print(")")
print()
print("# 範例2：使用75百分位切點+Caliper匹配的配置")
print("custom_results_robust = create_custom_psm_results(")
print("    binary_cutoff_method='percentile_75',")
print("    matching_method='caliper',")
print("    matching_ratio='1:1',")
print("    caliper_width=0.05,")
print("    with_replacement=False,")
print("    trimming_alpha=0.05,")
print("    treatment_focus='gov',")
print("    output_filename_suffix='robust_caliper'")
print(")")
print()
print("# 範例3：僅分析family變數的保守配置")
print("custom_results_family = create_custom_psm_results(")
print("    binary_cutoff_method='median',")
print("    matching_method='nearest', ")
print("    matching_ratio='1:1',")
print("    with_replacement=False,")
print("    trimming_alpha=0.0,")
print("    treatment_focus='family',")
print("    output_filename_suffix='family_conservative'")
print(")")
print()
print("基於您的比較結果分析，建議使用改進模型3（1:2匹配）配置：")
print("- GAP結果：0.0182*** (t=3.03)")
print("- GAP_S結果：0.0345*** (t=5.52)")
print("- 這個配置在統計顯著性和效應大小方面都表現最佳")
print(f"{'='*100}")
...
print("3. 自定義最佳模型功能：")
print("   - 使用 create_custom_psm_results() 函數自行設定最佳配置")
print("   - 建議配置：1:2匹配 + 5%修剪 + 中位數切點")
print("="*100)
...
try:
    # 使用最佳表現配置執行PSM分析
    custom_results_enhanced = create_custom_psm_results(
        binary_cutoff_method='median',
        matching_method='nearest',
        matching_ratio='1:2',
        with_replacement=False,
        trimming_alpha=0.05,
        treatment_focus='both',
        output_filename_suffix='enhanced'
    )
    
    if custom_results_enhanced:
        print("\n" + "="*80)
        print("Enhanced PSM檔案生成成功！")
        print("輸出檔案：")
        print("- PSM results/psm_results_enhanced_family.csv")
        print("- PSM results/psm_results_enhanced_gov.csv")
        print("="*80)
    else:
        print("\nEnhanced PSM分析失敗")
        
except Exception as e:
    print(f"\nEnhanced PSM分析過程中發生錯誤：{str(e)}")
    import traceback
    traceback.print_exc()